...
function gameStatus(){
    for (let i = 0; i < 9; i += 3){
        if (boardStatus[i] === boardStatus[i+1] && boardStatus[i+1] === boardStatus[i+2] && boardStatus[i+2] != 0){
            return {
                result: 'won',
                winMove: [i, i+1, i+2],
    ...
    for (let i = 0; i < 3; i++){
        if ( boardStatus[i] === boardStatus[i+3] && boardStatus[i+3] === boardStatus[i+6] && boardStatus[i+6] != 0){
            return {
                result: 'won',
                winMove: [i, i+3, i+6],
                winner: boardStatus[i]
    ...
    for (let i = 0; i < 3; i += 2){
        if (boardStatus[i] === boardStatus[4] && boardStatus[4] === boardStatus[4 + (4-i)] && boardStatus[4] != 0){
            return {
                result: 'won',
                winMove: [i, 4, 4 + (4-i)],
                winner: boardStatus[i]
